...
- Tunable repo variables: `DAILY_DD_BREAKER_PCT`, `MAX_PEAK_DD_PCT`, `CAPITAL_GSL_MODE`, `CAPITAL_GSL_DISTANCE_BUFFER`, `CAPITAL_GSL_FALLBACK_DISTANCE_PCT`, `CAPITAL_GSL_STOPLOSS_RETRY_MULTIPLIER`, `CAPITAL_GSL_ATR_MULT`, `CAPITAL_GSL_ATR_PERIOD`, `EXEC_REBALANCE_MODE`.
- **`EXEC_REBALANCE_MODE`** (`reset` default | `skip_unchanged`): the daily `--reset` close+reopen pays one full spread per position per day — it single-handedly kills low-vol strategies (bonds: 6.95%/yr drag at 5% vol target). `skip_unchanged` holds same-side positions within 20% size tolerance and only trades flips/drifts/exits. Validated 2026-07 (`validate_strategy_configs.py`): gold net Sharpe 0.54→0.79 full / 1.09→1.36 OOS, maxDD -10.2%→-7.2%. Caveat: a held position keeps its entry-day GSL distance until the next flip re-opens it.
